...
### End-to-end lifecycle demo (synbioGRS27)
...
# Option A: use the scaffolded workspace
pixi run crm workspace use synbiogrs27

# Option B: create a fresh workspace via CLI
...
# After you fill table IDs and set AIRTABLE_API_KEY
pixi run crm schema apply --mirror airtable
```

This creates `workspaces/synbiogrs27/local.sqlite` as the canonical database.

#### 3) Add sponsor and attendee leads
...
**Why table IDs?** Airtable explicitly recommends using table IDs so renaming a table doesn’t require code changes ([Airtable][4]).
You can find base/table/field IDs via Airtable URLs and tooling (IDs start with `app`, `tbl`, `fld`) ([Airtable Support][5]).
